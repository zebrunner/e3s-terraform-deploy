...
resource "aws_iam_role_policy" "codebuild_nexus_management" {
  count = var.automatic_update_enabled && var.nexus_enabled ? 1 : 0
  name  = "${local.e3s_codebuild_policy_name}-nexus-management"
  role  = aws_iam_role.codebuild[0].id
  policy = templatefile(
    "./iam_data/codebuild-nexus-management-policy.json",
    {
      account           = data.aws_caller_identity.current.account_id
      region            = var.region
      resource_prefix   = var.resources_prefix
      nexus_bucket_name = var.nexus_s3_bucket.name
    }
  )
}
